# -*- coding: utf-8 -*-
"""
pressure and temperature (ptu) history
...
def trata_dir(fs_dir: str):
    ...
    for lroot, ldirs, lfiles in os.walk(fs_dir, followlinks=False):
        ...
        for lfile in lfiles:
            ...
            lflow = lfile.lower()
            # PTU* history file ?
            if lflow.endswith(".his") and \
              (lflow.startswith("ptu") or lflow.startswith("pressure_temperature")):
                # trata file
...
def trata_line(f_mongo_client, fs_loc: str, flst_head: list, flst_line: list):
    ...
    db.save_data(f_mongo_client["ptuhis"], fs_loc, ldct_reg)
